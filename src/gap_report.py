...
def _closest_retrieved(searcher, question_text: str, top_k: int) -> GapRow:
    ...
    except Exception as exc:  # noqa: BLE001 — retrieval failure must not kill the report
        hits = []
    if not hits:
    ...
    label = "adjacent (distance {:.3f})".format(distance) if distance is not None else "retrieved (no distance)"
...
def render_markdown(report: dict) -> str:
    ...
    if report["weak"]:
        ...
        lines.append("")
    lines.append("_Generated by `qresp gap-report` — retrieval-only, no API calls, no model beyond the local embedding._")
